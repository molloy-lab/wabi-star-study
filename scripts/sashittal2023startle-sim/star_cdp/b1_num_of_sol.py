...
def write_star_cdp_sbatch(exe: str, cmat: str, priors: str, sbatch_file: str, search_space: str):
    ...
    rows_to_wrtie = ['#!/bin/bash', 
                    ...
                    ' '.join([time,'-v','-o','consensus_star_cdp_usage.log',exe,'-i',cmat, '-m', priors, '-x 0','-t',search_space, '-consensus','--output', 'star_cdp', '-o','consensus_star_cdp',\
...
def main():
    ...
    for folder in folders:
        ...
        for rep in reps:
            ...
            if not os.path.exists(os.path.join(cur_res_rep_path, "consensus_star_cdp_one_sol.tre")):
