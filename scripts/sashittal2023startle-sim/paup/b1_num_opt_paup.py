...
def write_paup_sbatch(curr_dir : str, paup_exe: str, paup_nex_file:str, sbatch_file:str):
    ...
    rows_to_wrtie = ['#!/bin/bash', 
                    ...
                    ' '.join([time, '-v', '-o', 'num_opt_paup_usage.log', paup_exe, paup_nex_file, '&>', 'num_opt_paup.log', '2>&1'])
                    ]
...
def submit_paup_sbacth(sbatch: str, data_prefix: str, res_dir:str):
    ...
    os.chdir(res_dir)
    if not os.path.exists(os.path.join(res_dir, 'num_opt_all_trees.trees')):
...
def main():
    ...
    for folder in folders:
        ...
        for rep in reps:
            ...
            paup_nex_file = os.path.join(cur_res_rep_path, 'paup_camsok_hsearch_save_all.nex')
            paup_sbatch = os.path.join(cur_res_rep_path, 'run_num_opt_paup.sbatch')
            data_prefix = folder + "/"+rep
            print(data_prefix)
            

            if not os.path.exists(os.path.join(cur_res_rep_path, "num_opt_all_trees.trees")):
