...
def main():
    ...
    for folder in folders:
        ...
        for rep in reps:
            ...
            paup_nex_file = os.path.join(cur_res_rep_path, 'paup_camsok_hsearch_fast.nex')
            paup_sbatch = os.path.join(cur_res_rep_path, 'run_paup.sbatch')
            data_prefix = folder + "/"+rep
            print(data_prefix)
            

            if not os.path.exists(os.path.join(cur_res_rep_path, "all_trees.trees")):
