...
def write_to_nexus(df:pd.DataFrame, paup_file:str, folder: str, binary_file:str, taxon_map_file:str, weights:list):
    ...
    all_trees_file = os.path.join(folder, 'num_opt_all_trees.trees')
    with open(paup_file, 'w') as fp:
        fp.write("#NEXUS\n")
        fp.write("BEGIN PAUP;\n")
        fp.write("set maxtrees=1000 increase=auto;\n")
        ...
        fp.write("hsearch start=1 swap=TBR nbest=all rseed=12345;\n")
        fp.write("rootTrees;\n")
...
def main():
    ...
    for folder in folders:
        ...
        for rep in reps:
            ...
            binary_file = os.path.join(cur_res_rep_path, 'binary.nex')

            taxon_map_file = os.path.join(cur_res_rep_path, 'taxon_map.csv')
            paup_file = os.path.join(cur_res_rep_path, 'paup_camsok_hsearch_save_all.nex')
        # print(cmat_df)
            cmat_df.astype({col: int for col in cmat_df.columns[1:]})
            binary_df, weights = binarize(cmat_df, priors)
            write_to_nexus(binary_df, paup_file, cur_res_rep_path, binary_file, taxon_map_file, weights)
            print("Finished writing paup nex file for " + cur_res_rep_path)
            exit(0)
