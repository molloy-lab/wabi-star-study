...
def main():
    ...
    result_dir = '/fs/cbcb-lab/ekmolloy/jdai123/star-study/bio_result/python_startle_nni'
    ...
    for folder in folders:
        ...
        nni_tree_path = os.path.join(cur_res_path, 'nni_tree.newick')
        data_prefix = folder + "/"
        print(data_prefix)

        if not os.path.exists(score_path):
            score_prefix = os.path.join(cur_res_path, 'nni_score')
            score_res = sp.run([startle_exe, 'small', cmat_path, priors_path, nni_tree_path, '--output', score_prefix], capture_output=True, text=True)
